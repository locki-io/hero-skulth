...
def build_twin(cfg: TwinConfig) -> tuple[Metabolism, EpochClock]:
    ...
    metabolism = Metabolism(
        treasury, daemon, StaticGate({"mock-vault": "cleared"}), CognitionMeter()
    )

    if venue_file.exists():
        if principal > 0:
            metabolism.venue = vault  # resume, don't re-deposit
    else:  # first breath: the one allocation decision of the demo slice
        if cfg.deposit_micro > 0:
            metabolism.deposit(vault, cfg.deposit_micro)
            decision_file.write_text(
                f"deposited {_usdc(cfg.deposit_micro)} USDC into mock-vault (gate cleared)"
            )
        else:
            decision_file.write_text("abstained — no venue entered (survival-first)")
